...
class UserDatabase {
	...
	public function addBattleNetProfile( $userId, $name, $realm, $region ) {
		...
		if( $profileId > 0 ) {		
			...
			$updateStmt = $this->db->prepare("
					...
					SET `CharacterRaceID`=? AND `CharacterClassID`=? AND `Level`=? AND `Serialized`=?"
			);
	...
	public function getProfile($id, $legacy = false) {
		...
		$record = DatabaseHelper::fetchOne(
				Database::getConnection(),
				"SELECT `UserID`, `Serialized` FROM chardev_user.`chardev_characters` WHERE `ID`=? ORDER BY `History` ASC",
				array($id)
		);
	...
	private function saveProfile( $serialized, $id = 0 ) {
		...
		if( $id > 0 ) {
			$profile = DatabaseHelper::fetchOne($this->db, "SELECT * FROM chardev_user.`chardev_characters` WHERE `Deleted`=0 AND `ID`=?",array($id));
		...
		DatabaseHelper::execute($this->db, "LOCK TABLES chardev_user.`chardev_characters` WRITE");
	
		$duplicate = DatabaseHelper::fetchOne($this->db, "SELECT * FROM chardev_user.`chardev_characters` WHERE Serialized=? AND UserID=?", array(serialize($serialized),$loggedInUser->getId()));
		if( $duplicate ) {
		...
		if( $id <= 0 ) {
			$idRecord = DatabaseHelper::fetchOne($this->db, "SELECT (max(`ID`)+1) AS `NextID` FROM chardev_user.`chardev_characters`");
			...
			DatabaseHelper::execute($this->db, "UPDATE chardev_user.`chardev_characters` SET `History` = `History` + 1 WHERE `ID` = ? ORDER BY `History` DESC", array($id));
		}
	
		DatabaseHelper::execute($this->db, "INSERT INTO chardev_user.`chardev_characters` VALUES (?,?,?,?,?,?,?,?,?,0,0)", array(
				$id,
				$loggedInUser->getId(),
				$name,
				$description,
